...
second line. Medians are necessary --- the basis count is high-variance, and a
mean over five seeds still puts $d=11$ below $d=10$.}
\label{tab:sweep}
\end{table}
...
Neither comparison is fair to its subject: Clarabel is built for large sparse
programs of arbitrary shape, and Welzl's recursion is being run in the least
favourable language for one. That the specialised method wins on its own problem
is the weakest possible claim, and the only one made here.
...
Table~\ref{tab:bench} lives in. One of their observations deserves flagging:
