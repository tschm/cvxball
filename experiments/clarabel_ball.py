"""The Clarabel cone-program route to the smallest enclosing ball.

This lived in :mod:`cvxball.solver` until Clarabel became a development-only
dependency. The package now ships one solver -- the pure-NumPy active-set method
-- and this route stays here, where it is what it has in fact become: the
reference the shipped method is measured against, in
:mod:`experiments.bench_seb` and in the tests that check the two agree.

The program is the direct translation of

    minimise r  subject to  ||p_i - x|| <= r,

with decision vector ``z = (r, x)`` and one second-order cone per point. Nothing
canonicalises it on the way: :func:`build_soc_program` assembles Clarabel's
``(P, q, A, b, cones)`` by hand, which is why this is worth keeping as a
reference -- it is the cone program itself, not a modelling layer's opinion of
it.

Input validation is imported from :mod:`cvxball.solver` rather than repeated, so
that the two routes reject exactly the same inputs with exactly the same
messages; the tests that parametrise over both would otherwise be comparing
error strings as well as answers.
"""
...
def min_circle_clarabel(points: np.ndarray, verbose: bool = False) -> tuple[float, np.ndarray]:
    ...
    The second-order-cone program is assembled by :func:`build_soc_program`;
    ...
    Example:
        ...
        array([0.5, 0.5])
    """
    p_mat, q, a_mat, b, cones = build_soc_program(_validate(points))
