...
These notes accompany \texttt{cvxball}, which implements both routes: the
second-order cone program of Section~\ref{sec:convex} handed to a conic solver,
and the active-set method of Section~\ref{sec:algorithm} in pure NumPy. They share
one interface, so they are interchangeable and can be tested against each other.
...
\begin{tabular}{ll}
  ...
  The cone program of \S\ref{sec:convex} & \texttt{\_build\_soc\_program} \\
...
degrades as $d$ grows. G\"artner~\cite{gaertner1999} made it robust, and
Fischer, G\"artner and Kutz~\cite{fischer2003} gave the combinatorial
``smallest enclosing ball of balls'' framework that the method of
Section~\ref{sec:algorithm} belongs to.
